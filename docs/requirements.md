...
- 版本输入支持 `m` 前缀简写：`m4.5.2` 等价于 `--edition mono 4.5.2`（内部归一化为 dotnet），
  所有接受版本的命令（install/remove/default/run）统一生效。
- 支持一次传入多个版本串行安装（如 `gdit install 4.5.2 m4.6.2`）；`--edition` 显式给出时统一
  应用于所有版本，任一失败不中断其余，最终按是否有失败汇总退出码。
...
### FR-02 默认版本与启动（P0 · 第二阶段）

`gdit default <版本>` 将已安装版本设为全局默认；`gdit default` 无参数显示当前默认。

- 通过原子更新 `~/.gdit/current` symlink 完成设置，失败保留旧链接。
- `gdit setup` 显式创建或修复 `~/.gdit/bin/godot` shim，但不修改 shell 配置或系统 PATH。
- PATH 中的 `godot` shim 指向 `gdit`，由 gdit 读取 `current`、启动真实引擎并透传参数、输出和
  退出码。shim 不访问网络。
- `gdit run` 启动引擎：`-d` 启动默认版本；`<版本>` 显式启动指定版本（不改变默认）；无参数且
  终端可用时交互选择已安装版本后启动，非 TTY 下报用法错误。
- `default` 与 `run` 只接受已完整安装的版本，不触发隐式下载。
- 版本输入支持 `m` 前缀（如 `gdit run m4.6.2`），与 `--edition` 同时出现时报用法错误。
- 默认版本对所有目录一致，不做项目级自动切换。

### FR-03 版本查看与卸载（P0 · 第一/二阶段）

- `gdit list` 列出已安装版本并标记当前默认版本。
- `gdit remove <版本>` 卸载指定版本；删除不可逆，TTY 下确认后执行（默认否），非 TTY 必须
  显式 `-y`/`--yes` 跳过确认。
- 当前默认版本不能直接卸载，必须先 `default` 到其他版本。
...
### FR-11 run 启动（P0 · 第二阶段，取代原 exec 设计）

`gdit run [<版本>|-d] [-- <参数>]` 使用指定或默认版本运行 Godot，透传参数、输出和退出码。

- `-d` 使用当前默认版本；无参数且终端可用时交互选择已安装版本后启动。
...
P0 包括 FR-01、FR-02、FR-03、FR-04、FR-05、FR-08 和 FR-11。**P0 全集是 MVP 的最终目标；第一阶段只
